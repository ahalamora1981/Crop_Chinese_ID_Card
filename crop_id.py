...
class Crop_ID():
    ...
    def crop(self, input_path, img_file):
        ...
        try:
            ...
            # 获得掩码图片
            mask_image = self.get_mask(img_path)

            # 用掩码图片获得黑底身份证图片
            black_bg_image = self.get_black_bg_image(image, mask_image)

            # 获得边框范围
            card_range = self.get_card_range(black_bg_image)

            # 判断边框是否为证件（如边框不是证件，则大概率是人像）
            if self.range_is_card(image, card_range):
                # 通过边框范围截取白底身份证图片
                cropped_image = self.get_cropped_image(image, card_range)

                # 如果高比宽长，就逆时针旋转90度
                image = self.get_rotate_image_90(cropped_image)
            ...
            if '正面' in img_path:
                ...
                if self.back_is_upside_down(image):
                    print(f'{img_path} 翻转180度')
                    image = self.get_rotate_image_180(image)
            ...
            # 输出图片
            cv2.imwrite(os.path.join(self.output_path, 'output_' + img_file), image)
    ...
    def get_black_bg_image(self, img, mask_img):
        original_image = img
    ...
    def get_card_range(self, image):  # 传入掩码图片
    ...
    def range_is_card(self, image, card_range):
    ...
    def get_cropped_image(self, image, card_range): # image 为白底图片, img_range 为左右上下四个点的字典
    ...
    def get_rotate_image_90(self, image):
        # 获取图像的高度和宽度
        height, width = image.shape[:2]
    ...
    def get_rotate_image_180(self, image):
        image = cv2.rotate(image, cv2.ROTATE_180)
            
        return image

    def back_is_upside_down(self, image):
        height, width = image.shape[:2]
        ...
        # 计算左右两个半区的红色像素数量
        red_pixels_left = self.count_red_pixels(left_half)
        red_pixels_right = self.count_red_pixels(right_half)
...
if __name__ == '__main__':
    ...
    for img_file in img_list:
        crop_id.crop(input_path, img_file)

    duration = time.time() - start
    print(f"Done within {duration} seconds.")